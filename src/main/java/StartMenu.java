    ...
    private void selectDifficulty(Difficulty difficulty) {
        ...
        for (int i = 0; i < difficulties.length; i++) {
            if (difficulties[i] == selectedDifficulty) {
                difficultyButtons[i].setBackground(new Color(0, 180, 0));
                difficultyButtons[i].setForeground(Color.BLACK);
            } else {
                difficultyButtons[i].setBackground(new Color(60, 60, 60));
                difficultyButtons[i].setForeground(Color.LIGHT_GRAY);
            }
        }
    }
}
